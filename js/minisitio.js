...
$('#dni').on('keyup', function(){
	...
	if(document.getElementById('documentacion').value==2 && document.getElementById('dni').value.length >= 10){
		...
		/*if(id != false){
			...
			crearMensajeError(this,"El numero de cédula o pasaporte ya existe");
		}*/
	}
	else if(document.getElementById('documentacion').value==3 && document.getElementById('dni').value.length >= 7){
		validarDocumento($('#dni').val(),$('#documentacion').val(),this);
		existeDni($('#dni').val());
	}
});
...
function validarDocumento(numero,tipo,obj/*,campoErr,campoSeccion,btn*/){  
