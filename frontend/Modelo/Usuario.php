...
WHERE
  ...
  public static function guardarUsuario($data,$tipo,$idUsuario=false){
  ...
  public static function buscaDatosUsuario($idUsuario){
    if(empty($idUsuario)){ return false; }
  ...
  public static function actualizarMetodo($metodo,$idusuario){
    if (empty($idusuario) || empty($metodo)){ return false; }
    return $GLOBALS['db']->update('mfo_usuariom2',array('metodo_resp'=>$metodo),'id_usuario='.$idusuario);
  }
}  
?>