...
class Modelo_Respuesta{
  ...
  public static function facetaSiguiente($idusuario){
    if (empty($idusuario)){ return false; }
    $sql = "SELECT f.orden FROM mfo_opcionm2 o 
            INNER JOIN mfo_preguntam2 p ON p.id_pregunta = o.id_pregunta
            INNER JOIN mfo_competenciam2 c ON c.id_competencia = p.id_competencia
            INNER JOIN mfo_facetam2 f ON f.id_faceta = c.id_faceta
            WHERE o.id_opcion = (SELECT MAX(id_opcion) FROM mfo_respuestam2 WHERE id_usuario = ?)";
    ...
    if (empty($result)){
      ...
      $sql = "SELECT id_faceta FROM mfo_facetam2 WHERE orden = ?";
